...
function statusClassName(status: string) {
  switch (status) {
    ...
    case "Rescheduled": return "bg-amber-500/20 text-amber-300 border border-amber-400/30";
    default:            return "bg-zinc-600/30 text-zinc-300 border border-zinc-500/30";
  }
}
...
export default function InterviewsPage() {
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            {iv.status === "Completed" && iv.candidates?.status === "Hired" && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold bg-emerald-500/20 text-emerald-400 border border-emerald-500/25">
                <UserCheck className="w-3 h-3" /> {t("interviews.hired")}
              </span>
            )}
            {iv.status === "Completed" && iv.candidates?.status === "Rejected" && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-semibold bg-red-500/20 text-red-400 border border-red-500/25">
                <UserX className="w-3 h-3" /> {t("interviews.rejected")}
              </span>
            )}
          </div>
        </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-xl font-semibold" style={{ color: "var(--ats-text-h)" }}>{t("interviews.title")}</h1>
        <p className="text-sm mt-0.5" style={{ color: "var(--ats-text-body)" }}>
          {loading ? t("common.loading") : `${interviews.length} ${t("interviews.interviewCount")}`}
        </p>
      </div>
      ...
      {calendarStatus === "disconnected" && (
        <div className="flex items-center justify-between gap-4 p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-indigo-300">{t("interviews.connectCalendar")}</p>
              <p className="text-xs text-indigo-400/70 mt-0.5">{t("interviews.connectDesc")}</p>
            </div>
          </div>
          <a
      ...
      {loading ? (
        ...
        <div className="py-24 text-center rounded-xl" style={{ background: "var(--ats-surface)", border: "1px solid var(--ats-border)" }}>
          ...
          <p className="text-sm font-medium" style={{ color: "var(--ats-text-body)" }}>{t("interviews.noInterviews")}</p>
          <p className="text-xs mt-1" style={{ color: "var(--ats-text-muted)" }}>{t("interviews.scheduleHint")}</p>
        </div>
      ) : (
          ...
          {upcoming.length > 0 && (
            <section>
              <h2 className="text-sm font-semibold uppercase tracking-wider mb-3" style={{ color: "var(--ats-text-muted)" }}>
                {t("interviews.upcoming")} ({upcoming.length})
              </h2>
