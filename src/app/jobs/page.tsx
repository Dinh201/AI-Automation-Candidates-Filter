  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-white">Vị trí tuyển dụng</h1>
          <p className="text-sm text-zinc-500 mt-0.5">Quản lý JD và cấu hình rubric chấm điểm</p>
        </div>
        <button
          onClick={openCreate}
        ...
        {TABS.map(({ label, value }) => (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium transition-colors relative ${
              activeTab === value ? "text-white" : "text-zinc-500 hover:text-zinc-300"
            }`}
          >
            {label}
